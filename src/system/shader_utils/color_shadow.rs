...
const COLOR_SHADOW_SHADER_VS: &'static str = "color_shadow_shader_vs";
const COLOR_SHADOW_SHADER_FS: &'static str = "color_shadow_shader_fs";
const COLOR_SHADOW_PILEPINE: &'static str = "color_shadow_pipeline";

pub struct ColorShadowShader;

#[setup]
impl ColorShadowShader {
	#[init]
	pub fn init(
		...
		mut static_index: WriteRes<BoxShadowStaticIndex>,
		device: Res<RenderDevice>,
	) {
		let shader = GlslShaderStatic::init(
			COLOR_SHADOW_SHADER_VS,
			COLOR_SHADOW_SHADER_FS,
			&mut shader_catch, 
			&mut shader_map, 
			||{include_str!("../../source/shader/color_shadow.vert")}, 
			||{include_str!("../../source/shader/color_shadow.frag")});
		...
		// 插入背景颜色shader的索引
		static_index.write(BoxShadowStaticIndex(r));
...
pub fn init_static(
	...
	let mut shader_static = create_shader_common_static(
		...
		shader,
		create_shader_info);
	shader_static.bind_group.insert(BLUR_GROUP, Share::new(create_blur_group_layout(device)));
...
pub fn create_blur_group_layout(device: &RenderDevice) -> BindGroupLayout {
		...
		entries: &[
			wgpu::BindGroupLayoutEntry {
				...
				ty: wgpu::BindingType::Buffer {
					...
					min_binding_size: wgpu::BufferSize::new(48), // color.rgba、 left、right、top、bottom矩形描述，每个方向为一个f32, 另外一个blur值也为f32， 因此大小为 9 * 4 = 36, 似乎需要对其，需要最小48
				},
				count: None,
			},
		],
...
#[derive(Deref)]
pub struct BoxShadowStaticIndex(pub StaticIndex);

pub const BLUR_GROUP: usize = 4;
