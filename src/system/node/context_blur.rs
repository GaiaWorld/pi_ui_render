...
use crate::{
    components::{calc::RenderContextMark, user::Blur},
...
pub fn blur_calc(
    mut query_set: ParamSet<(
        Query<(Entity, &Blur, &mut RenderContextMark), Changed<Blur>>,
        Query<&'static mut RenderContextMark>,
    )>,
    del: RemovedComponents<Blur>,
    ...
    // Blur修改，如果saturate、hue_rotate、bright_ness都为0， 则取消渲染上下文标记, 否则设置渲染上下文标记
    for (entity, blur, mut render_mark_value) in query_set.p0().iter_mut() {
        if **blur > 0.0 {
