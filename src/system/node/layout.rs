...
impl CalcLayout {
	...
	pub fn prepare_data(
		e: Event,
		mut query: Query<Node, Write<NodeState>>
	) {
	...
	fn get_up(&self, k: LayoutKey) -> Option<Up<LayoutKey>> {
		if k.text_index.is_null() {
			...
			return Some(Up::new(
				...
				LayoutKey {entity: if prev.is_null() {Id::<Node>::null()}else{k.entity}, text_index: prev},
				LayoutKey {entity: if next.is_null() {Id::<Node>::null()}else{k.entity}, text_index: next}
			));
		}
	}
