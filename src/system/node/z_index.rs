...
impl ZSort {
    fn new(z_index: isize, index: usize, node: Id<Node>, children_count: usize) -> Self {
        ZSort {
            z_index,
            index,
            node,
...
fn local_reset(
    query: &Query<Node, Option<&ZIndex>>,
    tree: &EntityTree<Node>,
    mark: &mut SecondaryMap<Id<Node>, usize>,
    ranges: &mut Query<Node, Write<ZRange>>,
    vec: &mut Vec<ZSort>,
    children_count: usize,
    mut zrange: ZRange,
) {
    fn empty(_mark: &mut SecondaryMap<Id<Node>, usize>, _node: &Id<Node>) {}
    zrange.start += Z_SELF;
    // 脏状态
    let mut dirty = Dirty {
        children_count,
...
fn range_set(
    ...
    children_count: usize,
    mut zrange: ZRange,
    func: fn(&mut SecondaryMap<Id<Node>, usize>, &Id<Node>),
) {
...
fn reset(
    ...
    zrange.start += Z_SELF;
    let len = vec.len();
    fn mark_remove(mark: &mut SecondaryMap<Id<Node>, usize>, node: &Id<Node>) {
        // 清理脏标志，这样层脏迭代器就不会弹出这个节点
        mark.remove(*node);
    }
    range_set(
...
mod test {
    ...
    use super::CalcZindex;

    // 初始化，将所有节点以根节点作为父节点组织为树
    fn init_tree(root: In<(Id<Node>, Id<Node>)>, mut tree: EntityTreeMut<Node>, entitys: Query<Node, Id<Node>>) {
        let (r, child) = root.0;
        if child.is_null() {
            for e in entitys.iter() {
                if e != r {
                    ...
                    tree.insert_child(e, Id::<Node>::null(), std::usize::MAX);
                }
            }
        } else {
    ...
    fn test() {
        ...
        init_tree_sys.run(In((root, Id::<Node>::null())));

        let mut query = world.query::<Node, (Id<Node>, Option<&ZIndex>, &ZRange)>();
    ...
    fn asset(
        world: &mut World,
        query: &mut QueryState<Node, (Id<Node>, Option<&ZIndex>, &ZRange)>,
        result: Vec<(isize, (usize, usize))>,
    ) {
        for (e, z, r) in query.iter_mut(world) {
            println!("=========, id:{:?}, z:{:?}, r: {:?}", e.offset(), z, r);
