...
use crate::{prelude::UserCommands, components::{calc::EntityKey, user::{serialize::{StyleTypeReader}, ClassName, StyleAttribute, style_attr_list_to_buffer}}, resource::{NodeCommand, FragmentCommand, CmdType}};
use bevy::{ecs::{system::SystemState}, prelude::{Entity, Resource, Commands, Plugin, World, IntoSystemConfig}};
use pi_bevy_ecs_extend::system_param::res::{OrInitResMut, OrInitRes};
use pi_null::Null;
use pi_slotmap::SecondaryMap;

use super::{system_set::UiSystemSet, node::user_setting, RunState};
...
impl Plugin for UiCmdTracePlugin {
    fn build(&self, app: &mut bevy::app::App) {
		...
		match self.option {
			TraceOption::Record => {
				app
				.add_system(cmd_record.in_set(UiSystemSet::Setting).before(user_setting::user_setting));
			},
			TraceOption::Play => {
				app
				.add_system(cmd_play.in_set(UiSystemSet::Setting).before(user_setting::user_setting));
			},
...
pub fn cmd_play(
	world: &mut World,
	state: &mut SystemState<(Commands, OrInitRes<Records>, OrInitResMut<PlayState>, OrInitResMut<UserCommands>)>,
) {
	...
	for fragment_commands in r.fragment_commands.iter() {
		let r: Vec<Entity> = fragment_commands.entitys.iter().map(|r| {
			if let None = play_state.get_node(r) {
				log::warn!("xxxxxxx=========={:?}", r);
			}
			play_state.get_node(r).unwrap()
		}).collect();
		cmds.fragment_commands.push(FragmentCommand { key: fragment_commands.key, entitys: r });
	}
	...
	for s in r.style_commands.iter() {
		...
		cmds.style_commands.commands.push((
			play_state.get_node(&s.entity).unwrap(),
			class_mate.start,
			class_mate.end,
		));
	}
