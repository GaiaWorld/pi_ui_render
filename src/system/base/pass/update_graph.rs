...
pub fn update_graph(
    mut pass_query: ParamSet<(
        Query<(&mut GraphId, Entity, &ParentPassId, &PostProcessInfo, &RenderContextMark)>,
        (
    ...
    for entity in mark_changed.iter() {
        ...
        if let Ok((mut graph_id, entity, parent_passs_id, post_info, mark)) = p0.get_mut(*entity) {
            ...
            if mark.any() {
                ...
                *graph_id = GraphId(graph_node_id);
                ...
                remove_node(**graph_id, &mut rg, ref_count);
