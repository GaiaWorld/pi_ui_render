...
impl Node for Pass2DNode {
	...
	fn build<'a>(
		...
		let p2 = param.query.p2();

		let (mut render_target, as_image, mut fbo_info) = match p2.get_mut(pass2d_id) {
			Ok(r) => r,
			_ => return Ok(out),
		};
		let as_image_level = get_as_image(as_image);
		...
		if let Some(catch_target) = catch_target {
			...
			if as_image_level != pi_style::style::AsImage::Force {
				log::trace!("as_image=============== {:?}", (pass2d_id, as_image_level));
				// 每帧都清理掉render_target.target， 避免握住无法释放
				render_target.target = StrongTarget::None;
			}
			// log::warn!("as_image=============== {:?}", (pass2d_id, pi_time::Instant::now() - t1));
			return Ok(out);
		}
		...
		// }
		// log::warn!("as_image1=============== {:?}", (pass2d_id, pi_time::Instant::now() - t1));
...
impl RenderTarget {
    ...
    pub fn get_or_create<'s, G: GetTargetView, T: Iterator<Item=G>>(
        ...
        as_image: pi_style::style::AsImage,
...
pub fn get_as_image(as_image: Option<&AsImage>) -> pi_style::style::AsImage {
	match as_image {
