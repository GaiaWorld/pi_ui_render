...
use pi_ecs::{
    monitor::Event,
    prelude::{res::WriteRes, FromWorld, QueryState, Res, World},
};
...
use pi_render::{
    components::view::target_alloc::{SafeAtlasAllocator, ShareTargetView, TargetDescriptor, TargetType, TextureDescriptor},
    graph::{
        node::{Node, NodeRunError},
        RenderContext,
    },
    rhi::{
        asset::RenderRes, bind_group::BindGroup, bind_group_layout::BindGroupLayout, buffer::Buffer, device::RenderDevice, dyn_uniform_buffer::Group,
        texture::ScreenTexture, CommandEncoder, RenderQueue,
    },
};
...
use crate::{
    components::{
        ...
        pass_2d::{Camera, Draw2DList, DrawIndex, ParentPassId, Pass2D, Pass2DKey, PostProcessList, RenderTarget, ScreenTarget, ViewMatrix},
        user::{Aabb2, Point2},
    },
    resource::{
        draw_obj::{ClearColorBindGroup, CommonSampler, CopyFboToScreen, DynBindGroups, DynFboClearColorBindGroup, ShareLayout},
        ClearDrawObj, Viewport,
    },
...
pub struct Param<'s> {
    pass2d_query: QueryState<
        Pass2D,
        (
            &'static Camera,
            Option<&'static ViewMatrix>,
            &'static Draw2DList,
            Option<&'static NodeId>,
            &'static ParentPassId,
        ),
    >,
...
impl Node for Pass2DNode {
    ...
    fn run<'a>(
        ...
        async move {
            ...
            if let Some((camera, _view_matrix, list, node_id, parent_pass2d_id)) = param.pass2d_query.get(&world, pass2d_id) {
    ...
    fn prepare<'a>(&'a self, _context: RenderContext) -> Option<BoxFuture<'a, Result<(), NodeRunError>>> { None }

    fn finish<'a>(&'a self, _context: RenderContext, _inputs: &'a [Self::Output]) -> BoxFuture<'a, Result<(), NodeRunError>> {
        async { Ok(()) }.boxed()
    }
}
...
impl InitGraphData {
    ...
    pub fn create_dyn_target_type(atlas_allocator: &SafeAtlasAllocator, view_port: &Viewport) -> DynTargetType {
        DynTargetType {
