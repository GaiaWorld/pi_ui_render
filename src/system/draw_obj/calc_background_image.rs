...
lazy_static! {
	...
	pub static ref BACKGROUND_TEXTURE_DIRTY2: StyleMarkType = BACKGROUND_TEXTURE_DIRTY1.clone().set_bit(StyleType::ObjectFit as usize) | &*LAYOUT_DIRTY;
}

pub fn background_texture_change(mark: SingleRes<GlobalDirtyMark>) -> bool {
	mark.mark.has_any(&*BACKGROUND_TEXTURE_DIRTY2)
}
...
#[derive(Default)]
pub struct BackgroundImageTemp (pub GridBufer, pub Vec<(Entity, (Range<usize>, Range<usize>))>);
...
pub fn calc_background_image_inner(
...
) -> Option<((Range<usize>, Range<usize>), usize/*rendercount*/, BoxType)> {
	let (entity, layout, background_image_texture_ref, background_image_clip, background_image_mod, background_image, sdf_slice, sdf_uv) = data;
	let sdf_uv = &sdf_uv.0;
	...
	let mut instance_count = 1;
	{
		...
		if background_image_mod.repeat.x == ImageRepeatOption::Stretch && background_image_mod.repeat.y == ImageRepeatOption::Stretch {
			...
			if !has_slice {
				...
				// if background_image.0.as_str().contains("icon_yingxiong") {
					// log::warn!("bg image========================{:?}", (entity , background_image.0.as_str(), [pstart.x, pend.x, pend.y, pend.y], [uv1.x, uv2.x, uv1.y, uv2.y], &padding_aabb));
				// }
				
				return Some(((len-4..len-2, len-2..len), instance_count, box_ty));
			}
		}
		// use pi_key_alloter::Key;
		// if background_image.0.as_str().contains("icon_yingxiong") {
		...
		instance_count = (y_range.len() / 2) * (x_range.len() / 2);
		// log::debug!("BackgroundImageEvent==============entity: {:?}, instance_count: {:?}, box_ty: {:?}, \nsdf_uv: {:?}, \nsdf_slice: {:?}", entity, instance_count, box_ty,  sdf_uv, sdf_slice);
		return Some(((x_range, y_range), instance_count, box_ty));
	}
}
