...
use pi_null::Null;
use pi_style::{style_parse::{Attribute, style_list_to_buffer}, style_type::ClassMeta};
...
impl FragmentMap {
    pub fn extend(&mut self, value: Fragments) {
		if value.fragments.len() + self.fragments.len() < value.fragments.capacity() {
			self.fragments.reserve(value.fragments.capacity() - ( value.fragments.len() + self.fragments.len()));
		}
