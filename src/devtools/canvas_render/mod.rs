...
use pi_bevy_render_plugin::{node::Node, PiRenderDevice, PiScreenTexture, SimpleInOut};
use pi_render::{components::view::target_alloc::ShareTargetView, renderer::sampler::SamplerRes, rhi::{bind_group::BindGroup, buffer::Buffer, device::RenderDevice, sampler::SamplerDesc, texture::PiRenderDefault, BufferInitDescriptor}};
use pi_share::Share;
...
impl Node for CanvasRendererNode {
    type BuildParam = (Query<'static, &'static mut SimpleInOut>, SingleRes<'static, PiScreenTexture>, SingleRes<'static, PiRenderDevice>);
    type ResetParam = ();
	...
	fn build<'a>(
        ...
        if self.data.is_none() {
            let surface_format = wgpu::TextureFormat::pi_render_default();
            let device = (**param.2).clone();
            let device1 = &**device;
        ...
        if from.len() > 0 {
            for entity in from.iter() {
                let input = match param.0.get(*entity) {
                    Ok(r) => r,
                    Err(_) => continue,
                };
                let target = input.target.as_ref().unwrap();
                if data.objs.get(i).is_none() {
                    data.objs.push(CanvasRenderer::default());
                }
                let obj = &mut data.objs[i];
                obj.try_change(&data.device, target, &data.bindgroup_layout);
                i += 1;
            }
        }
		Ok(())
    ...
    fn run<'a>(
        ...
        if from.len() > 0 {
            ...
            rpass.set_vertex_buffer(0, data.vertex.slice(..).deref().clone());
            let mut i = 0;
