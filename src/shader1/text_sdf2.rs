...
#[sampler(Filtering)]
pub struct DataTexSampBind;


#[derive(BindLayout, BindingType)]
#[layout(set(3), binding(1))]
#[texture(dim(D2), multi(false), kind(Float))]
pub struct UIndexTexBind; // storagetexture: TODO


#[derive(BindLayout, BindingType)]
#[layout(set(4), binding(1))]
#[texture(dim(D2), multi(false), kind(Float))]
pub struct UDataTexBind; // storagetexture: TODO


#[derive(BindLayout, BindingType)]
#[layout(set(3), binding(0))]
#[sampler(Filtering)]
pub struct IndexTexSampBind;


#[derive(Input)]
#[location(0)]
pub struct AGlyphVertexVert;


#[derive(Input)]
#[location(1)]
pub struct IndexInfoVert;


#[derive(Input)]
#[location(2)]
pub struct TranslationVert;


#[derive(Input)]
#[location(3)]
pub struct DataOffsetVert;
...
impl ShaderProgram for ProgramMeta {
    fn create_meta() -> pi_render::rhi::shader::ShaderMeta {
        ...
        super::super::shader::depth::push_meta(_meta, _visibility, &[]);
        super::super::shader::camera::push_meta(_meta, _visibility, &[]);
        ...
        _meta.add_binding_entry(
            4,
            (
                DataTexSampBind::as_layout_entry(_visibility),
                BindingExpandDescList::new(vec![BindingExpandDesc::new_sampler("data_tex_samp")], merge_defines(_defines, &[])),
            ),
        );
        ...
        _meta.add_binding_entry(
            3,
            (
                UIndexTexBind::as_layout_entry(_visibility),
                BindingExpandDescList::new(vec![BindingExpandDesc::new_texture("u_index_tex")], merge_defines(_defines, &[])),
            ),
        );
        ...
        _meta.add_binding_entry(
            4,
            (
                UDataTexBind::as_layout_entry(_visibility),
                BindingExpandDescList::new(vec![BindingExpandDesc::new_texture("u_data_tex")], merge_defines(_defines, &[])),
            ),
        );
        ...
        _meta.add_binding_entry(
            3,
            (
                IndexTexSampBind::as_layout_entry(_visibility),
                BindingExpandDescList::new(vec![BindingExpandDesc::new_sampler("index_tex_samp")], merge_defines(_defines, &[])),
            ),
        );
        ...
        super::super::shader::ui_meterial::push_meta(_meta, _visibility, &[]);
        push_vs_code(&mut meta.vs);
        push_fs_code(&mut meta.fs);
        meta.varyings = ShaderVarying(vec![
            InOut::new("uv", "vec2", 5, vec![]),
            InOut::new("lp", "vec2", 6, vec![]),
            InOut::new("u_index_info", "vec4", 7, vec![]),
            InOut::new("u_data_offset", "vec2", 8, vec![]),
            InOut::new("u_info", "vec4", 9, vec![]),
        ]);
        meta.ins = ShaderInput(vec![
            InOut::new("a_glyph_vertex", "vec4", 0, vec![]),
            InOut::new("index_info", "vec4", 1, vec![]),
            InOut::new("translation", "vec2", 2, vec![]),
            InOut::new("data_offset", "vec2", 3, vec![]),
            InOut::new("info", "vec4", 4, vec![]),
        ]);
...
fn push_vs_code(_codes: &mut BlockCodeAtom) {
    ...
    super::super::shader::camera::push_code(_codes, merge_defines(_defines, &[]).as_slice());
    super::super::shader::depth::push_code(_codes, merge_defines(_defines, &[]).as_slice());
    super::super::shader::ui_meterial::push_code(_codes, merge_defines(_defines, &[]).as_slice());
...
"
    ...
    gl_Position = project * view * world * pos1;
...
vec2 get_index_uv(vec2 nominal_size)
{
	...
	return (nominal_size * uv + offset) / textureSizeOrBottomLeftBorder;
}
...
void main() {
	mat4 u_gradient = clipSdfOrSdfline;
	vec4 uColor = color;
	vec4 u_outline = strokeColorOrURect;
	// vec2 index_tex_size = textureSizeOrBottomLeftBorder;
