		...
		if let Ok(r) = input.try_parse(|input| {
			...
			match f.as_ref() {
				"scale" => input.parse_nested_block(|input| {
					let s = input.expect_number()?;
					Ok(TransformFunc::Scale(s, match input.try_parse(|input| {input.expect_number()}) {
						Ok(r) => r, 
						Err(_) => s,
					}))
				}),
...
fn parse_text_shadow<'i, 't>(input: &mut Parser<'i, 't>) -> Result<TextShadows, ParseError<'i, ValueParseErrorKind>> {
...
fn test1() {
	...
	if let Err(r) = parse_class_map_from_string(s, &mut class_sheet) {
	}
