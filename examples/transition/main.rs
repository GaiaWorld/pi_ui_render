...
impl Example for TransitionExample {
    fn init(&mut self, mut world: Param, size: (usize, usize)) {
		...
		world.user_cmd.set_style(div1, PositionLeftType(Dimension::Points(0.0)));
        ...
        world.user_cmd.set_style(div1, TransitionDurationType(smallvec![Time(3000)]));
		world.user_cmd.set_style(div1, TransitionDelayType(smallvec![Time(0)]));
		world.user_cmd
		...
		world.user_cmd.set_style(div2, PositionLeftType(Dimension::Points(0.0)));
        ...
        world.user_cmd.set_style(div2, TransitionDurationType(smallvec![Time(3000)]));
    ...
    fn render(&mut self, cmd: &mut UserCommands) {
		self.frame += 1;
		if self.frame == 60 * 3 {
			cmd.set_style(*self.transition_node1, PositionLeftType(Dimension::Points(300.0)));

			cmd.set_style(*self.transition_node2, PositionLeftType(Dimension::Points(300.0)));
			cmd.set_style(*self.transition_node2, WidthType(Dimension::Points(150.0)));
		}
