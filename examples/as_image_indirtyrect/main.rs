...
impl Example for QuadExample {
    fn init(&mut self, mut world: Param, size: (usize, usize)) {
        ...
        world.user_cmd.set_style(div3, PositionLeftType(Dimension::Points(150.0)));
        world.user_cmd.set_style(div3, PositionTopType(Dimension::Points(150.0)));
        world.user_cmd
            .set_style(div3, BackgroundColorType(Color::RGBA(CgColor::new(1.0, 0.0, 1.0, 1.0))));
        world.user_cmd.append(div3, root);
        // 设置改变节点
        self.change_node = EntityKey(div3);
    }

    fn render(&mut self, cmd: &mut UserCommands) {
        // 不停的转动change_node
        self.rotate += 1.0;
        cmd.set_style(self.change_node.0, RotateType(self.rotate));
