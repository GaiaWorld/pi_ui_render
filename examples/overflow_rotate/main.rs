...
use pi_style::{style_type::{
    BackgroundColorType, HeightType, MarginLeftType, MarginTopType, OpacityType, OverflowType, PositionLeftType, PositionTopType, PositionTypeType,
    TransformType, WidthType,
}, style::{Point2, Aabb2}};
use pi_ui_render::{
    components::{user::{CgColor, ClearColor, Color, Transform, TransformFunc, RenderDirty, Viewport}, NodeBundle, calc::EntityKey},
...
impl Example for QuadExample {
    fn init(&mut self, world: &mut World, size: (usize, usize)) {
		...
		self.cmd.set_style(div3, BackgroundColorType(Color::RGBA(CgColor::new(1.0, 0.0, 0.5, 1.0))));
        ...
        self.cmd.set_style(div6, BackgroundColorType(Color::RGBA(CgColor::new(0.7, 0.7, 0.7, 1.0))));
        // 设置opacity，测试Pass2d在父上存在TransformWillChange的情况下能否正确渲染
        ...
        self.cmd.set_style(div7, BackgroundColorType(Color::RGBA(CgColor::new(0.0, 1.0, 0.0, 1.0))));
        ...
        self.cmd.set_style(div8, BackgroundColorType(Color::RGBA(CgColor::new(1.0, 1.0, 0.0, 1.0))));
		let mut transform = Transform::default();
        transform.funcs.push(TransformFunc::RotateZ(45.0)); // 旋转45度
