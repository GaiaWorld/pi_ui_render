...
use pi_style::style_type::{WidthType, HeightType, BackgroundColorType, PositionTypeType, PositionLeftType, PositionTopType, MarginLeftType, MarginTopType, TransformWillChangeType, OpacityType};
...
impl Example for QuadExample {
    async fn init(
		...
		gui.gui.set_style(div1, TransformWillChangeType(transform_willchange));
		...
		gui.gui.set_style(div3, WidthType(Dimension::Points(100.0)));
		gui.gui.set_style(div3, HeightType(Dimension::Points(200.0)));
		// 设置TransformWillChange，向右平移100个像素
		let mut transform_willchange = TransformWillChange::default();
		transform_willchange.0.funcs.push(TransformFunc::TranslateX(50.0));
		gui.gui.set_style(div3, TransformWillChangeType(transform_willchange));
