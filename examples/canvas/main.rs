...
use pi_null::Null;
use pi_ui_render::{
	components::user::{Color, CgColor, Viewport, ClearColor, RenderTargetType, Canvas}, 
	export::Engine, utils::cmd::NodeCmd
};
use pi_style::{style_type::{WidthType, HeightType, BackgroundColorType, PositionTypeType, PositionLeftType, PositionTopType, MarginLeftType, MarginTopType}, style::{Aabb2, Point2, Node}};
...
impl Example for QuadExample {
    async fn init(
		...
		gui.gui.push_cmd(NodeCmd(Viewport(Aabb2::new(Point2::new(0.0, 0.0), Point2::new(size.0 as f32, size.1 as f32))), root_one));
		...
		gui.gui.push_cmd(NodeCmd(Viewport(Aabb2::new(Point2::new(0.0, 0.0), Point2::new(300.0, 300.0))), root_tow));
		...
		gui.gui.set_style(div1, BackgroundColorType (Color::RGBA(CgColor::new(0.0, 1.0, 0.0, 1.0)) ));
